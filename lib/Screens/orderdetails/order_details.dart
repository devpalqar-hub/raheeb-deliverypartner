...
class OrderDetailsScreen extends StatelessWidget {
      ...
      body: GetBuilder<TrackingController>(
        builder: (trackCtrl) {
          ...
          return SingleChildScrollView(
            ...
            child: Column(
              children: [
                _buildTrackingTimeline(trackCtrl, orderController),
                const SizedBox(height: 16),
                _buildCustomerCard(),
                const SizedBox(height: 16),
                _buildNextStepCard(orderController),
                const SizedBox(height: 16),
  ...
  Widget _buildTrackingTimeline(
    TrackingController trackCtrl,
    OrderController orderController,
) {
  final currentStatus =
      trackCtrl.tracking?.data?.status ?? "processing";
    ...
    child: Column(
      ...
      children: [
        ...
        ...orderFlow.asMap().entries.map((entry) {
          ...
          bool isCompleted =
              _isStepCompleted(step["status"]!, trackCtrl);
...
Widget _buildCustomerCard() {
  ...
  final String phone =
      (shipping.phone != null && shipping.phone!.isNotEmpty)
          ? shipping.phone!
          : customer.phone;
  ...
  Widget _buildNextStepCard(OrderController orderController) {
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        style: ElevatedButton.styleFrom(
          backgroundColor:
              isDelivered ? Colors.green :Color(0xff2F80ED),
          foregroundColor: Colors.white,
          padding:
        ...
        child:
            Text(isDelivered ? "Delivered" : "Mark as Delivered"),
      ),
    );
  }
   ...
   return _cardContainer(
     child: Column( 
      ...
      children: [ Row( children: 
       ...
       color: Colors.orange),
        SizedBox(width: 8),
         Text( "Issues with delivery?", 
         style: TextStyle(fontWeight: FontWeight.bold), ), ], ), 
         const SizedBox(height: 8), const 
         Text( "If you can’t complete this order due to vehicle issues or emergency, you can transfer it.", ),
          const SizedBox(height: 12), 
        ...
        this.isLast = false, });
         ...
         if (isCompleted || isActive) circleColor = Colors.blue;
          return Row( crossAxisAlignment: CrossAxisAlignment.start,
           children: [ Column( children: [ if (!isFirst) 
           Container(width: 2, height: 20, color: Colors.grey.shade300),
            ...
            const SizedBox(width: 12), Expanded( child: 
            Padding( padding: const EdgeInsets.only(bottom: 20), 
            child: Column( crossAxisAlignment: CrossAxisAlignment.start,
             children: [ Text(title, style: 
             TextStyle( fontWeight: isActive ? FontWeight.bold : FontWeight.w500)),
              const SizedBox(height: 4),
               Text(subtitle, 
               style: TextStyle( 
                fontSize: 13, color: isActive ? Colors.blue : Colors.grey)),
                 ], ), ),
                  ), ], ); }
